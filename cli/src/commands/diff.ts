...
function computeDiff(local: string, remote: string): { added: number; removed: number } {
...
function printUnifiedDiff(local: string, remote: string, localLabel: string, remoteLabel: string): void {
  const localLines = local.split("\n");
  const remoteLines = remote.split("\n");
  ...
  const hunks: { kind: "same" | "add" | "remove" | "change"; localIdx?: number; remoteIdx?: number; line: string }[] = [];
...
export async function diff(name: string): Promise<void> {
  const localFile = resolve(`${name}.prompt.md`);

  // Check for local file
  if (!existsSync(localFile)) {
    console.error(
      chalk.red(`Error: Local file not found: ${relative(cwd(), localFile)}`) +
  ...
  try {
    const remote = await apiGet<DiffResponse>(`/api/prompts/${encodeURIComponent(name)}`);
    ...
    console.log(
      chalk.yellow(`${diffStats.removed} line(s) removed, ${diffStats.added} line(s) added.`)
    );
    console.log(chalk.dim(`  Remote version: ${remote.version}, updated: ${remote.updatedAt}`));
    ...
    if (err instanceof ApiError) {
      if (err.status === 404) {
        console.error(
          chalk.red(`  Prompt "${name}" does not exist on the server.`) +
            "\n  Use " +

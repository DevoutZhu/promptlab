...
export async function pull(name, options) {
    const spinner = ora(`Pulling prompt "${chalk.cyan(name)}"...`).start();
    try {
        const prompt = await apiGet(`/api/prompts/${encodeURIComponent(name)}`);
        ...
        spinner.succeed(chalk.green(`Pulled "${prompt.name}" (v${prompt.version})`) +
            chalk.dim(` -> ${filename}`));
        if (prompt.updatedAt) {
        ...
        if (err instanceof ApiError) {
            if (err.status === 404) {
                console.error(chalk.red(`  Prompt "${name}" not found.`) +
                    "\n  Use " +

...
export async function push(filePath) {
    ...
    try {
        ...
        spinner.succeed(chalk.green(`Pushed "${result.name}" (v${result.version})`));
        ...
        if (err instanceof ApiError) {
            console.error(chalk.red(`  HTTP ${err.status} on ${err.path}`) +
                (err.message ? `\n  ${err.message}` : ""));
            if (err.status === 401) {
                console.error(chalk.yellow("  Hint: Authentication failed. Set your token via") +
